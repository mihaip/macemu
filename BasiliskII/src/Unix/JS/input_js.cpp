...
void ReadJSInput() {
  ...
  if (lock) {
    ...
    int has_mouse_move = EM_ASM_INT_V({
      return workerApi.getInputValue(
          workerApi.InputBufferAddresses.mouseMoveFlagAddr);
    });
    if (has_mouse_move) {
      int dx = EM_ASM_INT_V({
        return workerApi.getInputValue(
            workerApi.InputBufferAddresses.mouseMoveXDeltaAddr);
      });

      int dy = EM_ASM_INT_V({
        return workerApi.getInputValue(
            workerApi.InputBufferAddresses.mouseMoveYDeltaAddr);
      });
    ...
    int has_ethernet_interrupt = EM_ASM_INT_V({
      return workerApi.getInputValue(
          workerApi.InputBufferAddresses.ethernetInterruptFlagAddr);
    });
